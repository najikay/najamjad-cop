...
One console script per repo (`najamjad-cop` here, `najamjad-thief` in the
...
uv run najamjad-cop peer                       # go online: MCP server + tunnel + dashboard
uv run najamjad-cop peer --no-tunnel --no-dashboard   # local play, nothing exposed

uv run najamjad-cop replay <log.json>          # re-hash every step, print the verdict
uv run najamjad-cop replay <log.json> --serve  # open the viewer instead
uv run najamjad-cop archive match.zip          # bundle the evidence (secrets excluded)
```
...
Rate limits live in `config/rate_limits.json`; the cop's game configs live under
`config/police/` (kept strictly separate from the thief per book rules 1-2).
