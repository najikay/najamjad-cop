...
The **police/cop side** of Team NajAmjad's final project for *Orchestration of AI Agents*:
...
> **Companion repository (thief agent):** https://github.com/najikay/najamjad-thief
...
[![ci](https://github.com/najikay/najamjad-cop/actions/workflows/ci.yml/badge.svg)](https://github.com/najikay/najamjad-cop/actions/workflows/ci.yml)
...
```bash
git clone https://github.com/najikay/najamjad-cop.git
cd najamjad-cop
...
|---|---|
| `port 8802 … already in use` | Another agent is running. Stop it, or change `network.my_port` in `config/police/game.toml`. |
...
| Opponent reports us unreachable | Check the tunnel: `uv run najamjad-cop preflight`. A `502` means the agent is not running; a JSON-RPC error such as *"Client must accept text/event-stream"* means it **is** healthy — that is a browser hitting an MCP endpoint. |
| `Failed to spawn: najamjad-cop` | Run from the repository root; the console script lives in this repo's `.venv`. |
...
| `config/police/game.toml` | **Private, local.** Our port, opponent URL, tunnel hostname, LLM choices, belief tuning. Never crosses the network. |
...
| Key | Effect |
|---|---|
| `network.my_port` | Our MCP port (8802 cop / 8801 thief, so both run locally). |
...
[Running it](#running-it) §2 for the panels and the optional controls.
